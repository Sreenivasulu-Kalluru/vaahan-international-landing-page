...
export default function InnovationSection() {
  ...
  return (
    <section id="innovation" ref={containerRef} className="py-32 relative overflow-hidden">
      {/* Background tech pattern */}
      <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, currentColor 1px, transparent 0)', backgroundSize: '40px 40px' }}></div>
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <motion.div style={{ y: y1 }} className="space-y-8">
            <div className="bg-muted/30 backdrop-blur-md p-8 rounded-3xl border border-border hover:border-primary/50 transition-colors">
          ...
          <motion.div style={{ y: y2 }} className="h-full">
            <div className="h-full min-h-[400px] rounded-3xl bg-linear-to-br from-primary/40 to-blue-600/40 border border-border relative overflow-hidden flex items-center justify-center p-8">
              {/* Abstract representation of tech */}
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.div 
                  animate={{ rotate: 360 }}
                  transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                  className="w-[300px] h-[300px] rounded-full border border-dashed border-foreground/30 absolute"
                />
                <motion.div 
                  animate={{ rotate: -360 }}
                  transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                  className="w-[200px] h-[200px] rounded-full border border-foreground/40 absolute"
                />
                <motion.div 
